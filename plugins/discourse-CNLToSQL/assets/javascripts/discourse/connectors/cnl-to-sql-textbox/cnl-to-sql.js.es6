...
export default ({
  ...
  actions: {
    ...
    convert(){
        ...
        //this.send('insertSQL');
        this._editor.getSession().setValue(this.get("query"));
    },
    ...
    makeSQLQueryForTags(){
      ...
      var q = 
            ...
            SELECT DISTINCT t.USER_ID AS user_id, post.CREATED_AT AS granted_at, post.ID AS post_id
            FROM TOPICS t
            INNER JOIN twt
    ...
    insertSQL(){
      var SQLEditor = document.getElementByTagName("ace-editor");
      alert(SQLEditor);
    },
